...
function MemberList({state,  updateState}) {
...
const getMemberList = async () => {
  try{
    const data = await fetch(import.meta.env.VITE_URL_TOCHANGE).then(res => res.json())
    console.log(data)
   setMembers(data)
  } catch (err) {
    console.log(err)
        ...
        <div className="flex flex-col m-5 h-auto  md:grid  grid-cols-3 gap-4 justify-center ">
          {members.map(member => (
              <div className="flex flex-row justify-center md:justify-around   " key={member._id}>
                ...
                <button onClick={(event) => deleteMember(member._id, event)} className="inline-flex items-center p-0.5 ml-2 text-sm text-indigo-400 bg-transparent rounded-sm hover:bg-indigo-200 hover:text-indigo-900 dark:hover:bg-indigo-800 dark:hover:text-indigo-300" data-dismiss-target="#badge-dismiss-indigo" aria-label="Remove">
